...
export function About() {
  return (
    <section id="about" className="mx-auto max-w-4xl px-6 py-24">
      ...
      <p className="mb-10 max-w-3xl text-lg text-zinc-300">
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {Object.entries(skills).map(([group, items]) => (
          <div key={group}>
            <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-violet-300">{group}</h3>
            <ul className="flex flex-wrap gap-2">
              {items.map((s) => (
                <li key={s} className="rounded-full bg-white/10 px-2.5 py-0.5 text-sm text-zinc-200">{s}</li>
              ))}
            </ul>
          </div>
        ))}

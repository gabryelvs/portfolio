...
<Claim>
62 tests. 59 are integration tests that run the real application against a real PostgreSQL, started once per test run with Testcontainers and exercised through Spring's MockMvc. Nothing about locking or transactions is mocked. The other 3 are unit tests for the JWT service. The suite grew from 55 to 62 during the final review: seven tests across the four fixes. GitHub Actions runs it on every push to main and every pull request.
...
- [The concurrent-move test](https://github.com/gabryelvs/taskboard-api/blob/d5d852447f437738e815d2fa66cbaf287f652640/src/test/java/com/gabryel/taskboard/card/CardMoveTest.java#L90-L121)
